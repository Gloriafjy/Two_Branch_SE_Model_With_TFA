...
def json_ext(path, flag):
    if flag is 'train':
        dataset_path = os.path.join(path, 'train', 'noisy')
        json_path = os.path.join(path, 'Json', 'train')
        os.makedirs(json_path, exist_ok=True)
    else:
        dataset_path = os.path.join(path, 'valid', 'noisy')
        json_path = os.path.join(path, 'Json', 'valid')
        os.makedirs(json_path, exist_ok=True)
